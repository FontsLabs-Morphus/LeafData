...
class LoadMySQLOperator(BaseOperator, Source):
    """
    Streaming pull from MySQL.
...
# ...
        ...
        if not self.sql or not self.sql.strip():
            raise MorphusAirflowException(
                message="MySQL SQL query cannot be empty",
                task_id=self.task_id,
                error_source="LoadMySQLOperator.__init__"
            )
    ...
    def _source_descriptor(self) -> dict:
        return {
            ...
            "header_less_file": str(
                self.header_less_file).strip().lower() in ("yes", "true", "1") if self.header_less_file is not None else True,
    ...
    def _setup_connection(self):
        """
        Establish MySQL streaming cursor and initialize headers eagerly.
        """
        self.log.info("Connecting to MySQL (streaming cursor)...")

        try:
            hook = MySqlHook(mysql_conn_id=self.mysql_conn_id, local_infile=True)
            conn = hook.get_conn()
            cursor = conn.cursor()
            ...
            self.log.info(f"MySQL headers detected: {self._db_headers}")
            return cursor, conn
            ...
            raise MorphusAirflowException(
                ...
                task_id=self.task_id,
                error_source="LoadMySQLOperator._setup_connection",
    ...
    def _fetch_data_stream(self, cursor):
        try:
            ...
            raise MorphusAirflowException(
                message="Error while fetching data from MySQL cursor",
                task_id=self.task_id,
                error_source="LoadMySQLOperator._fetch_data_stream",
                original_exception=e
            ) from e
        ...
        try:
            ...
            if self.source_job_type == PROCESS:
                ...
                def process_iter():
                    ...
                    yield ["SUCCESS"]
                def row_iter():
                    yield self._db_headers
                    yield from self._fetch_data_stream(cursor)
    # 2) then the data rows
                ...
                log_source_metrics(
                    ...
                    rows_written=2,  # header + SUCCESS row
            ...
            if not self._db_headers:
                raise MorphusAirflowException(
                    message="No headers detected from MySQL cursor. Cannot transform data.",
                    task_id=self.task_id,
                    error_source="LoadMySQLOperator.execute"
                )
...
class PushMySQLOperator(BaseOperator, Destination):
    """
    Push CSV into MySQL.
    ...
    def _setup_connection(self):
        self.log.info("Connecting to MySQL Database...")
        try:
            hook = MySqlHook(mysql_conn_id=self.mysql_conn_id, local_infile=True)
            conn = hook.get_conn()
            cur = conn.cursor()
            ...
            raise MorphusAirflowException(
                ...
                task_id=self.task_id,
                error_source="PushMySQLOperator._setup_connection",
                original_exception=e
            ) from e
    ...
    def _table_exists(self, hook) -> bool:
        try:
            ...
            raise MorphusAirflowException(
                message="Failed to check if MySQL table exists",
                task_id=self.task_id,
                error_source="PushMySQLOperator._table_exists",
                original_exception=e
            ) from e
    ...
    def _get_table_columns(self, hook) -> list:
        try:
            ...
            raise MorphusAirflowException(
                message="Failed to fetch MySQL table columns",
                task_id=self.task_id,
                error_source="PushMySQLOperator._get_table_columns",
                original_exception=e
            ) from e
    ...
    def _create_table_from_header(self, hook, header: list[str]) -> None:
        if not header:
            raise MorphusAirflowException(
                message="CSV header missing. Cannot create table.",
                task_id=self.task_id,
                error_source="PushMySQLOperator._create_table_from_header"
            )
        ...
        try:
            self.log.info(
                "Creating table %s.%s dynamically from CSV header.",
                self.schema_name,
                self.table_name,
            )
            hook.run(ddl)
            ...
            raise MorphusAirflowException(
                message="Failed to create MySQL table from CSV header",
                task_id=self.task_id,
                error_source="PushMySQLOperator._create_table_from_header",
                original_exception=e
            ) from e
    ...
    def _drop_table(self, hook):
        try:
            ...
            self.log.warning(
                "Dropping table %s.%s (overwrite mode).",
                self.schema_name,
                self.table_name
            )
            hook.run(sql)
            ...
            raise MorphusAirflowException(
                message="Failed to drop MySQL table",
                task_id=self.task_id,
                error_source="PushMySQLOperator._drop_table",
                original_exception=e
            ) from e
    ...
    def _read_file(self, file_path, delimiter=","):
        try:
            ...
            raise MorphusAirflowException(
                message="Failed to read CSV file for MySQL load",
                task_id=self.task_id,
                error_source="PushMySQLOperator._read_file",
                original_exception=e
            ) from e
        ...
        try:
            ...
            if not rows:
                raise MorphusAirflowException(
                    ...
                    task_id=self.task_id,
                    error_source="PushMySQLOperator._upload_data"
                )
            ...
            self.log.info("Executing LOAD DATA LOCAL INFILE")
            hook.run(sql)
            ...
            raise MorphusAirflowException(
                message="Failed while uploading data to MySQL",
                task_id=self.task_id,
                error_source="PushMySQLOperator._upload_data",
                original_exception=e
            ) from e
    ...
    def execute(self, context: Context):
        ...
        try:
            ...
            log_execution(
                ...
                run_id=run_id,
                status="FAILED",
                error_source="PushMySQLOperator.execute",
                error_message=str(e),
                root_cause=repr(e)
            )
...
LoadMySQLlOperator = LoadMySQLOperator   # optional (typo-safe)
PushMySQLlOperator = PushMySQLOperator   # optional (typo-safe)

LoadMySqlOperator = LoadMySQLOperator
PushMySqlOperator = PushMySQLOperator

LoadMySqlOperator = LoadMySQLOperator    # keep exactly this one for utils.py
PushMySqlOperator = PushMySQLOperator